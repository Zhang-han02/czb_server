...
public class DeptServiceImpl implements DeptService {
    ...
    public void create(Dept resources) {
        if (deptRepository.existsByDeptNumAndIdNot(resources.getDeptNum(), -1L)) {
            throw new BadRequestException(String.format("%s 部门编号已存在", resources.getDeptNum()));
        }
    ...
    public void update(Dept resources) {
        if (deptRepository.existsByDeptNumAndIdNot(resources.getDeptNum(), resources.getId())) {
            throw new BadRequestException(String.format("%s 部门编号已存在", resources.getDeptNum()));
        }
        // 旧的部门
...
//        resources.setId(dept.getId());
    ...
    public void download(List<DeptDto> deptDtos, HttpServletResponse response) throws IOException {
        ...
        for (DeptDto deptDTO : deptDtos) {
            ...
            map.put("部门编号", deptDTO.getDeptNum());
            if (deptDTO.getPid() != null){
                Dept parent = deptRepository.findById(deptDTO.getPid()).orElse(null);
                if (parent != null) {
                    map.put("上级部门", parent.getName());
                    map.put("上级部门编号", parent.getDeptNum());
                }
            }

            map.put("备注1", deptDTO.getRemark1());
            map.put("备注2", deptDTO.getRemark2());
    ...
    @Override
    public List<DeptDto> queryAllList(DeptQueryCriteria criteria) {
        Sort sort = Sort.by(Sort.Direction.ASC, "deptSort");
    ...
    public void importExcel(InputStream excelStream, int importModel) {
        ...
        readAll.forEach(item -> {
            String deptNum = CzbHelper.defaultString(item.get("部门编号"));
            Dept old = deptRepository.findByDeptNum(deptNum);
            ...
            dept.setName(CzbHelper.defaultString(item.get("名称")));
            String pDeptNum = CzbHelper.defaultString(item.get("上级部门编号"));
            if(StringUtils.isNotBlank(pDeptNum)){
                Dept pDept = deptRepository.findByDeptNum(pDeptNum);
                if (pDept != null) {
                    dept.setPid(pDept.getId());
                }
            }

            dept.setRemark1(CzbHelper.defaultString(item.get("备注1")));
            dept.setRemark2(CzbHelper.defaultString(item.get("备注2")));
    ...
    @Override
    public Dept findByDeptNum(String deptNum) {
        if (StringUtils.isBlank(deptNum)) {
            return null;
        }
        return deptRepository.findByDeptNum(deptNum);
    }
