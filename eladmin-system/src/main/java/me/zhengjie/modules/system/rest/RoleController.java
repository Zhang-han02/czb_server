...
public class RoleController {
    ...
    @ApiOperation("查询角色")
    @GetMapping
    @PreAuthorize("@el.check('roles:list','clientPermission:list')")
    ...
    @Log("修改角色客户端权限")
    @ApiOperation("修改角色客户端权限")
    @PutMapping(value = "/permission")
    @PreAuthorize("@el.check('clientPermission:list')")
    public ResponseEntity<Object> updatePermission(@ApiParam(required = true,value = "角色id") @RequestParam Long roleId,
                                                   @ApiParam(required = true,value = "权限id集合") @RequestBody List<Long> permissionIds){
        RoleDto role = roleService.findById(roleId);
        getLevels(role.getLevel());
        roleService.updatePermission(role,permissionIds);
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
